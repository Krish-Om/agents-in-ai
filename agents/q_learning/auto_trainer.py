#!/usr/bin/env python3
"""
Auto Trainer for Snake Game using Q-Learning
Trains the agent by directly playing the source.py game with Q-learning integration
"""
...
class SnakeQTrainer:
    ...
    def train(self, max_episodes=2000):
        """Main training loop"""
        print("🚀 Starting Q-Learning Training on Source Game")
        print(f"📊 Max Episodes: {max_episodes}")
        print(f"🧠 Learning Rate: {LEARNING_RATE}")
        print(f"💰 Discount Factor: {DISCOUNT_FACTOR}")
        print(f"🎯 Epsilon: {INITIAL_EPSILON} → {FINAL_EPSILON}")
        print("=" * 50)
        ...
        for episode in range(max_episodes):
            ...
            while step_count < max_steps:
                try:
                    ...
                    # Small delay to make training visible (optional)
                    if episode % 100 == 0:  # Show every 100th episode
                        time.sleep(0.01)
...
if __name__ == "__main__":
    trainer = SnakeQTrainer()
    trainer.train(max_episodes=1500)  # Adjust episodes as needed
